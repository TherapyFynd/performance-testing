...
test('Practice  login and  onboarding ', async ({ request }) => {
  ...
  const data = await generatePasswordlessLoginLink({
    ...
    email: "z7knk.pn4zm@inbox.testmail.app",
    request: request,
  });
...
//       });
  ...
  myEmails.billerEmail= Bookinginbox1!;
  ...
  myEmails.schedulerEmail= Bookinginbox5!;
  console.log(myEmails);
 ...
 await page.getByRole('button', { name: 'Next' }).nth(1).click();
  ...
  myEmails.intakeadmin1Email= Bookinginbox6!;
  console.log(myEmails);
...
//   } catch (error) {
//     console.log('Failed to find first locator, trying second locator')
//     await page.locator('#root > div._layout_10ldc_1 > div._content_10ldc_7 > div > div._filterTabContainer_1uhg9_24 > div > div > div._tabTwo_1ykd6_21._active_1ykd6_25').click();
...
//   await page.waitForTimeout(2000);
  ...
  await page.getByTestId('SendOutlinedIcon').click();
  await page.waitForTimeout(3000);
  await page.getByText('Supervisor 1').click();
...
test('DP Update and Logout', async () => {
    ...
    await page.locator('div').filter({ hasText: 'Practice' }).nth(3).click();
