...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.couplemaleclient?.length) {
...
test('Client Portal login and onboarding ', async ({ request }) => {
  ...
  const data = await generatePasswordlessLoginLink({
    email: myEmails.couplemaleclient!,
  ...
  await page.getByPlaceholder('Enter first name').fill('Rakesh');
...
test('Payment tab -Primary Therapist' ,async () => {
...
test('Chat tab -Primary Therapist' ,async () => {
...
test('Personal Infomation- Primary Therapist', async () => {
...
test('Couple Session with Primary Therapist',async () =>  {
  ...
  await page.locator('button:nth-child(1)').first().click();
        ...
        test('Couple Booking Appoinment -Primary Therapist', async () => {
          ...
          await page.getByRole('button', { name: 'Book appointment' }).nth(1).click();
          await page.getByLabel('Rakesh + Poornima').check();
          await page.getByLabel('Select service').click();
