...
const responseLogsFile = path.join(logsDir, 'therapist13-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist13?.length) {
...
// Main test cases
test.describe('All Therapist13 Role Test Cases', () => {
  test('Therapist13 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 13";
    ...
    }, "Therapist 13 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('13');
