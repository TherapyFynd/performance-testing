...
const responseLogsFile = path.join(logsDir, 'therapist24-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist24?.length) {
...
// Main test cases
test.describe('All Therapist24 Role Test Cases', () => {
  test('Therapist24 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 24";
    ...
    }, "Therapist 24 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('24');
