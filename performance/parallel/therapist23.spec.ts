...
const responseLogsFile = path.join(logsDir, 'therapist23-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist23?.length) {
...
// Main test cases
test.describe('All Therapist23 Role Test Cases', () => {
  test('Therapist23 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 23";
    ...
    }, "Therapist 23 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('23');
