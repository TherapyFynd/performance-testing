...
const responseLogsFile = path.join(logsDir, 'therapist3-responses.txt');
...
// Main test cases
test.describe('All Therapist3 Role Test Cases', () => {
  test('Therapist3 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 3";
    ...
    }, "Therapist 3 Navigate to login page", rolePrefix);
    ...
    }, "Terms and condition", rolePrefix);
    await page.locator('#root > div > div > div > div._stickyHeader_8mx9g_22 > div._tiltleNavigation_8mx9g_39 > button > svg > path').click();
