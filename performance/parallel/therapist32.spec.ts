...
const responseLogsFile = path.join(logsDir, 'therapist32-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist32?.length) {
...
// Main test cases
test.describe('All Therapist32 Role Test Cases', () => {
  test('Therapist32 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 32";
    ...
    }, "Therapist 32 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('32');
