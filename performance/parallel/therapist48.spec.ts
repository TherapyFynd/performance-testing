...
const responseLogsFile = path.join(logsDir, 'therapist48-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist48?.length) {
...
// Main test cases
test.describe('All Therapist48 Role Test Cases', () => {
  test('Therapist48 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 48";
    ...
    }, "Therapist 48 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('48');
