...
const responseLogsFile = path.join(logsDir, 'therapist41-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist41?.length) {
...
// Main test cases
test.describe('All Therapist41 Role Test Cases', () => {
  test('Therapist41 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 41";
    ...
    }, "Therapist 41 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('41');
