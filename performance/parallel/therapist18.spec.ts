...
const responseLogsFile = path.join(logsDir, 'therapist18-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist18?.length) {
...
// Main test cases
test.describe('All Therapist18 Role Test Cases', () => {
  test('Therapist18 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 18";
  ...
  }, "Therapist 18 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('18');
