...
const responseLogsFile = path.join(logsDir, 'therapist19-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist19?.length) {
...
// Main test cases
test.describe('All Therapist19 Role Test Cases', () => {
  test('Therapist19 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 19";
    ...
    }, "Therapist 19 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('19');
