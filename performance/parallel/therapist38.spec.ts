...
const responseLogsFile = path.join(logsDir, 'therapist38-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist38?.length) {
...
// Main test cases
test.describe('All Therapist38 Role Test Cases', () => {
  test('Therapist38 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 38";
    ...
    }, "Therapist 38 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('38');
