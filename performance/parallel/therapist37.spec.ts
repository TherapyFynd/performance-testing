...
const responseLogsFile = path.join(logsDir, 'therapist37-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist37?.length) {
...
// Main test cases
test.describe('All Therapist37 Role Test Cases', () => {
  test('Therapist37 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 37";
    ...
    }, "Therapist 37 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('37');
