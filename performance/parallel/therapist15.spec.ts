...
const responseLogsFile = path.join(logsDir, 'therapist15-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist15?.length) {
...
// Main test cases
test.describe('All Therapist15 Role Test Cases', () => {
  test('Therapist15 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 15";
  ...
  }, "Therapist 15 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('15');
