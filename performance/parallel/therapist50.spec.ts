...
const responseLogsFile = path.join(logsDir, 'therapist50-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist50?.length) {
...
// Main test cases
test.describe('All Therapist50 Role Test Cases', () => {
  test('Therapist50 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 50";
    ...
    }, "Therapist 50 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('50');
