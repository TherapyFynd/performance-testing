...
const responseLogsFile = path.join(logsDir, 'therapist10-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist10?.length) {
...
// Main test cases
test.describe('All Therapist10 Role Test Cases', () => {
  test('Therapist10 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 10";
    ...
    }, "Therapist 10 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('10');
