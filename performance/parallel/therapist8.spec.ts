...
test.describe('All Therapist8 Role Test case ', () => {
  // test.describe.configure({ mode: 'parallel' });
  test('Therapist8 login and  onboarding ', async ({ request }) => {
    const myEmails: IEmail = await readEmails();

     // Add "Owner Team" prefix to the log
     const rolePrefix = "Therapist 8";

    await measureActionTime(async () => {
      const data = await generatePasswordlessLoginLink({
        email: myEmails.therapist8!,
        request: request,
      });
    ...
    }, "Therapist 8 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('8');
