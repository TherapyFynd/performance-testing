...
const responseLogsFile = path.join(logsDir, 'therapist28-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist28?.length) {
...
// Main test cases
test.describe('All Therapist28 Role Test Cases', () => {
  test('Therapist28 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 28";
    ...
    }, "Therapist 28 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('28');
