...
const responseLogsFile = path.join(logsDir, 'therapist30-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist30?.length) {
...
// Main test cases
test.describe('All Therapist30 Role Test Cases', () => {
  test('Therapist30 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 30";
    ...
    }, "Therapist 30 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('30');
