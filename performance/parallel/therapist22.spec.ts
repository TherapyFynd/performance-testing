...
const responseLogsFile = path.join(logsDir, 'therapist22-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist22?.length) {
...
// Main test cases
test.describe('All Therapist22 Role Test Cases', () => {
  test('Therapist22 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 22";
    ...
    }, "Therapist 22 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('22');
