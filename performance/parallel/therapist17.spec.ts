...
const responseLogsFile = path.join(logsDir, 'therapist17-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist17?.length) {
...
// Main test cases
test.describe('All Therapist17 Role Test Cases', () => {
  test('Therapist17 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 17";
  ...
  }, "Therapist 17 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('17');
