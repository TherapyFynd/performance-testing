...
const responseLogsFile = path.join(logsDir, 'therapist33-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist33?.length) {
...
// Main test cases
test.describe('All Therapist33 Role Test Cases', () => {
  test('Therapist33 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 33";
    ...
    }, "Therapist 33 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('33');
