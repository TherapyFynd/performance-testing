...
const responseLogsFile = path.join(logsDir, 'therapist27-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist27?.length) {
...
// Main test cases
test.describe('All Therapist27 Role Test Cases', () => {
  test('Therapist27 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 27";
    ...
    }, "Therapist 27 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('27');
