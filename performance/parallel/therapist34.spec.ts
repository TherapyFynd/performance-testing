...
const responseLogsFile = path.join(logsDir, 'therapist34-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist34?.length) {
...
// Main test cases
test.describe('All Therapist34 Role Test Cases', () => {
  test('Therapist34 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 34";
    ...
    }, "Therapist 34 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('34');
