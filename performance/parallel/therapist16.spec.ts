...
const responseLogsFile = path.join(logsDir, 'therapist16-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist16?.length) {
...
// Main test cases
test.describe('All Therapist16 Role Test Cases', () => {
  test('Therapist16 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 16";
    ...
    }, "Therapist 16 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('16');
