...
const responseLogsFile = path.join(logsDir, 'therapist47-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist47?.length) {
...
// Main test cases
test.describe('All Therapist47 Role Test Cases', () => {
  test('Therapist47 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 47";
    ...
    }, "Therapist 47 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('47');
