...
const responseLogsFile = path.join(logsDir, 'therapist36-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist36?.length) {
...
// Main test cases
test.describe('All Therapist36 Role Test Cases', () => {
  test('Therapist36 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 36";
    ...
    }, "Therapist 36 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('36');
