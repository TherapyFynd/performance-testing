...
const responseLogsFile = path.join(logsDir, 'therapist49-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist49?.length) {
...
// Main test cases
test.describe('All Therapist49 Role Test Cases', () => {
  test('Therapist49 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 49";
    ...
    }, "Therapist 49 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('49');
