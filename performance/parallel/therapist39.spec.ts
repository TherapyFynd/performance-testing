...
const responseLogsFile = path.join(logsDir, 'therapist39-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist39?.length) {
...
// Main test cases
test.describe('All Therapist39 Role Test Cases', () => {
  test('Therapist39 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 39";
    ...
    }, "Therapist 39 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('39');
