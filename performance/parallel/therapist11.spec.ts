...
const responseLogsFile = path.join(logsDir, 'therapist11-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist11?.length) {
...
// Main test cases
test.describe('All Therapist11 Role Test Cases', () => {
  test('Therapist11 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 11";
  ...
  }, "Therapist 11 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('11');
