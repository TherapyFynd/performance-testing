...
const responseLogsFile = path.join(logsDir, 'therapist12-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist12?.length) {
...
// Main test cases
test.describe('All Therapist12 Role Test Cases', () => {
  test('Therapist12 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 12";
    ...
    }, "Therapist 12 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('12');
