...
const responseLogsFile = path.join(logsDir, 'therapist26-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist26?.length) {
...
// Main test cases
test.describe('All Therapist26 Role Test Cases', () => {
  test('Therapist26 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 26";
    ...
    }, "Therapist 26 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('26');
