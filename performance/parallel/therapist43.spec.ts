...
const responseLogsFile = path.join(logsDir, 'therapist43-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist43?.length) {
...
// Main test cases
test.describe('All Therapist43 Role Test Cases', () => {
  test('Therapist43 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 43";
    ...
    }, "Therapist 43 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('43');
