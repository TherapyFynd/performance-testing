...
const responseLogsFile = path.join(logsDir, 'therapist40-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist40?.length) {
...
// Main test cases
test.describe('All Therapist40 Role Test Cases', () => {
  test('Therapist40 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 40";
    ...
    }, "Therapist 40 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('40');
