...
const responseLogsFile = path.join(logsDir, 'therapist20-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist20?.length) {
...
// Main test cases
test.describe('All Therapist20 Role Test Cases', () => {
  test('Therapist20 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 20";
    ...
    }, "Therapist 20 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('20');
