...
const responseLogsFile = path.join(logsDir, 'therapist14-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist14?.length) {
...
// Main test cases
test.describe('All Therapist14 Role Test Cases', () => {
  test('Therapist14 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 14";
    ...
    }, "Therapist 14 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('14');
