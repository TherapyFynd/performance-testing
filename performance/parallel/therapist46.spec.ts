...
const responseLogsFile = path.join(logsDir, 'therapist46-responses.txt');
...
test.beforeAll(async ({ browser }) => {
  ...
  if (!myEmails?.therapist46?.length) {
...
// Main test cases
test.describe('All Therapist46 Role Test Cases', () => {
  test('Therapist46 login and onboarding', async ({ request }) => {
    const myEmails: IEmail = await readEmails();
    const rolePrefix = "Therapist 46";
    ...
    }, "Therapist 46 Navigate to login page", rolePrefix);
    await page.getByPlaceholder('Enter last name').click();
    await page.getByPlaceholder('Enter last name').fill('46');
